	...
	while (true)
	{
		...
		case '1':
			for (int stu = 0; stu < 2; stu++)
			{
				...
				cout << "이름 : ";
				InputValue(student[stu].StdName);
				cout << "학번 : ";
				Inputvalue(student[stu].Hakbun);
				cout << "\n\n";
				...
				for (int sub = 0; sub < 3; sub++)
				{
					...
					cout << "교과목명 : ";
					InputValue(student[stu].Sub[sub].SubName);
					cout << "과목 학점 수 : ";
					Inputvalue(student[stu].Sub[sub].Hakjum);
					cout << "과목등급(A+ ~ F) : ";
					InputValue(student[stu].Sub[sub].Grade);
